...
export default {
    ...
    methods: {
        ...
        show(position){
            this.isShow = true;
            if(this.float){
                $('.card-icon-material').css({'position':'absolute','max-width':this.maxWidth+'px',
                'max-height':this.maxHeight+'px','z-index':'9999',top:position.top,left:position.left,'box-shadow':'0px 3px 6px #00000029' });
            }
        },
        hide(){
            this.isShow = false;
