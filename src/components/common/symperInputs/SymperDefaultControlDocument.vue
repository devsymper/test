<template>
    <div>
                ...
                <div class="add-controls">
                    <div style="position:relative">
                        <div style="float:left">
                            <v-icon @click="removeControl(item)" style="float:left;margin-top:7px;margin-left:15px" small>mdi-close</v-icon>
                            <v-autocomplete
                                :items="listControls"
                                v-model="item.name"
                                dense
                                item-text="name"
                                color="black"
                                style="width:75%;margin:0px;float:left;padding-left:9px"
                                @change="handleValueChange"
                            >
                                <template  v-slot:item="data">
                                    <span class="fs-12">{{ data.item.title }}</span>
                                    <span class="fs-11 ml-3" style="color:gray">
                                        {{ data.item.name }} 
                                    </span>
                                </template>
                                <template v-slot:selection="data">
                                    <span class="fs-12 text-ellipsis" style="width:80%;display: contents;">{{ data.item.title }} 
                                    </span>
                                </template>
                            </v-autocomplete>
                        </div>
                       
                        <span @click="openLargeValueEditor(item)" style="position:absolute;right:5px;top:4px"><v-icon small>mdi-dock-window</v-icon></span>
                    </div>
        ...
        <v-btn class="mt-1" depressed small @click="addControls">
            Add
        </v-btn>
...
export default {
    ...
    methods:{
        ...
        openLargeValueEditor(item) {
            let el=this.listControls.find(element =>element.name==item.name);
            if (el) {
                item.title=el.title;
            }
        ...
        async getControlDocument(docId){
            const self = this;
            await documentApi.detailDocument(docId)
            .then(res => {
                if (res.status === 200) {
            ...
            .catch(err => {
                console.log("Error get control document",err)
            })
        },
            ...
            for (let index = 0; index < tables.length; index++) {
                ...
                if (controls.length>0) {
                    for (let j = 0; j < controls.length; j++) {
                        let obj = tableNames.find(element => element == controls[j].name);
                        if (!obj) {
