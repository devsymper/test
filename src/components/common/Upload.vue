<template>
    <div class="w-100 text-center">
        <label for="sym-upload-input" id="upload-segment" :style="{'background-image': 'url('+img+')'}">
            <v-icon v-if="!!!img" class="display-2 pickFileIcon">mdi-file-image</v-icon>
            <input 
                type="file" ref="image" 
        ...
        <v-btn 
            small color="primary" 
            class="mb-3" 
            :disabled="!!!img" @click.prevent="selectIcon"
        >
    ...
    methods: {
        ...
        changeFile(evt) {
            ...
            if(file.type.match(/image.*/)) {
                let tgt = evt.target || window.event.srcElement, self = this;
                if (FileReader) {
