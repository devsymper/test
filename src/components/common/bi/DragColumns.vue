...
export default {
    ...
    methods:{
        ...
        removeItem(item){
            let column = this.columns.find(ele => ele.columnName == item.columnName);
        ...
        checkShowItem(item){
            ...
            if(item && item.title && item.columnName){
                ...
                return item.title.toLowerCase().includes(s) || item.columnName.toLowerCase().includes(s);
            }
            return false;
        },
        change(e){
            if (e.added) {
                ...
                let obj = this.columns.filter(ele => ele.columnName == item.columnName);
                if (obj && obj.length > 1) {
