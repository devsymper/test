    ...
    <div>
       ...
       <v-row>
           <v-col class="d-flex flex-row justify-end">
               <v-btn  v-if="typeView=='add'" class="fs-13 fw-400 " color="success" @click="addKey()">
                    {{$t('authen.view.save')}}
               </v-btn>
                <v-btn  v-else class="fs-13 fw-400 " color="success" @click="updateKey()">
  ...
  methods: {
    async addKey(){
        ...
        if(res.status==200){
            this.$snotify({
                type: "success",
                title: this.$t("authen.view.add_title")+ this.$t(+"authen.notify.success"),
            });
    ...
    async updateKey(){
        ...
        if(res.status==200){
            this.$snotify({
                type: "success",
                title: this.$t("authen.view.update_title")+ this.$t(+"authen.notify.success"),
            });
            this.$emit('close-panel');
        }else{
            this.$snotify({
