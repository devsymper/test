<template>
    <v-row style="margin-top:5px">
        <v-col cols="2">
            <v-row> 
                <v-list-item-avatar style="margin-top:-10px" class="mx-2" v-if="!item.icon">
        ...
        <v-col cols="10" style="padding:6px!important" @click="openNotification(item)" >
                ...
                <v-col style="margin-top:-8px" class="text-ellipsis" >
                    <v-tooltip bottom>
                        <template v-slot:activator="{ on }">
                            <span v-on="on" style="margin-left:-12px" class="notification-item-title ">
            ...
            <v-row class="notification-item-info" style="margin-top:-5px">
                <v-col cols="6" class="text-ellipsis">
                    <v-tooltip bottom>
                         <template v-slot:activator="{on}">
                            <v-icon  v-on="on"  class="mr-2" size="12">{{$i('input.'+getScope(item.action))}}</v-icon>
                            <span  v-on="on" >{{item.extraLabel?item.extraLabel:''}} {{item.extraValue?item.extraValue:''}}</span>
                            </template>
