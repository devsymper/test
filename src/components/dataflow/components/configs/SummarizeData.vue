<template>
	<div class="d-flex flex-column summalize-config fs-13 mt-2">
		<v-text-field solo append-icon="mdi-magnify" v-model="searchColumns"> </v-text-field>
		...
		<div class="list-groupby-column mb-2" style="height:30%">
			...
			<div class="drag-columns">
				<VuePerfectScrollbar style="max-height: 130px">
					<draggable @change="handleChangeColumn" :list="nodeData.configs.aggColumns" :group="{ name: 'drop-column' }" class="drag-columns-area">
						<div
							:ref="'left_comp'"
						...
						>
							<v-menu open-on-hover top offset-y>
								<template v-slot:activator="{ on, attrs }">
									<span :style="{ width: column.inputWidth }" v-bind="attrs" v-on="on" class="column-as text-ellipsis" @dblclick="editColumnAs(idx)" v-show="!column.edit">
										{{ column.agg }}
									</span>
								</template>
								<v-list>
									<v-list-item v-for="(agg, name) in aggTpl[column.type]" :key="name" class="list-item-on-menu" @click="changeAggFunc({ column: column, aggName: name, idx: idx })">
										<v-list-item-title>
											{{ agg.name }}
										</v-list-item-title>
									</v-list-item>
