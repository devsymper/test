<template>
    <div :style="{height:extraInfo.height+'px'}" ref="unionBuilderWrapper" class="union-builder-component">
        <!-- <el-select 
            ref="columnSelect" 
            size="mini" 
            v-show="swapCellBySelect.showSelect" 
            filterable 
            v-model="swapCellBySelect.tmpSelectValue" 
            ...
            <el-option-group
                ...
                <el-option
                    v-for="item in swapCellBySelect.mismatches"
                    :key="item.index"
                    :label="item.title"
                    :value="item.index">
                    
                    <i :class=" 'pr-1 '+getColumnIcon(item.type)"></i>
                    <span v-html="item.columnName"></span>
                    <span v-html="item.title" class="float-right ml-2"></span>
            ...
            <el-option-group
                ...
                <el-option
                    v-for="item in swapCellBySelect.matches"
                    :key="item.index"
                    :label="item.title"
                    :value="item.index">
                    ...
                    <i :class=" 'pr-1 '+getColumnIcon(item.type)"></i>
                    <span v-html="item.columnName"></span>
                    <span v-html="item.title" class="float-right"></span>
...
function cellRenderer(params){
    ...
    return `
        <i title="`+col.type+`" class=" pr-1 mdi `+ DATATYPE_ICONS[col.type]+`"></i>
        <span title="`+col.columnName+`">`+params.value+`</span>`;
}
...
export default {
    ...
    computed:{
        ...
        getRowData(){
            ...
            this.nodeData.configs.tableConfig.forEach((element,idx) => {
                let newRow = _.cloneDeep(element);
    ...
    mounted() {
        ...
        this.agContainer = $(this.$refs.unionBuilderWrapper).find(".ag-center-cols-container");
    },
    methods:{
        getColumnIcon(type){
            return this.__dataIcon.iconPrefix+this.__dataIcon.columnTypeIcon[type];
        },
        showInvalidRow(rows,showNotify = true){
            ...
            if(this.checkInvalidRows(rows)){
                setTimeout((rows,thisCpn,showNotify) => {
                    for(let id of rows){
                        thisCpn.agContainer.find('.ag-row[row-id='+id+']').addClass('union-invalid-row');   
                    }
                    if(showNotify){
                        thisCpn.$notify({
                            title: 'Not match datatype columns!',
                            message: 'You need reorder cells in red rows',
                            type: 'warning',
                            duration: 5000
                        });
                    }
                }, 100,rows,this,showNotify);
            }
        },
        ...
        handleScrollTable(){
            if(this.checkInvalidRows(this.invalidRowsInfo)){
                ...
                this.invalidRowsDebounce = setTimeout((thisCpn) => {
        ...
        onRowDragMove(evt){
            ...
            if(this.prevRow > 0){
                ...
                row.find('.ag-cell[col-id='+this.dragColumnId+']').removeClass('will-drop-cell');   
            }
            this.prevRow = evt.overNode.rowIndex;
            row = this.agContainer.find('.ag-row[row-id='+this.prevRow+']');
            row.find('.ag-cell[col-id='+this.dragColumnId+']').addClass('will-drop-cell');
        },
        onRowDragEnd(evt){
            
            this.agContainer.find('.ag-row[row-id='+this.prevRow+']').find('.ag-cell[col-id='+this.dragColumnId+']').removeClass('will-drop-cell');   
            this.agContainer.find('.ag-row[row-id='+evt.node.rowIndex+']').find('.ag-cell[col-id='+this.dragColumnId+']').removeClass('will-drop-cell');
            if(this.dragColumnName == 'columnNumber'){
                this.reOrderConfigsRows(evt.node.rowIndex);
                return;
            }
            this.swapTwoFactor(evt.node.rowIndex,evt.overNode.rowIndex,this.dragColumnName);    
        },
        ...
        swapTwoFactor(startRow,endRow,colName,emit = true){
            ...
            this.swapValueTwoCells(startRow,endRow,colName.replace('dataset','col'));
            if(emit){
        ...
        swapValueTwoCells(startRow,endRow,colName){
            ...
            this.nodeData.configs.tableConfig[startRow.rowIndex][colName] = endValue;
            this.nodeData.configs.tableConfig[endRow.rowIndex][colName] = startValue;
        },
        marCellWillDrop(){
        ...
        handleCellFocus(info){
            ...
            if(info.column){
                ...
                this.swapCellBySelect.columnName = colKey.replace('dataset','col');
        ...
        handleChangeSelectValue(newVl){
            this.swapTwoFactor(this.swapCellBySelect.currentRowIdx,newVl,this.swapCellBySelect.columnName.replace('col','dataset'));
            this.swapCellBySelect.showSelect = false;
        },
        classifyCell(colOrderName,searchText = false){
            ...
            for(let idx in this.nodeData.configs.tableConfig){
                ...
                if(!_.isEmpty(row[colOrderName])){
                    let isMatched = false;
                    for(let name in row){
                        if(datasetCol !=name && name.includes('dataset') && row[name]){
                            isMatched = true;
                            break;
                        }
                    }
                    ...
                    if(searchText){
                        ...
                        if(row[colOrderName].columnName.toLowerCase().includes(searchText) || row[colOrderName].title.toLowerCase().includes(searchText)){
