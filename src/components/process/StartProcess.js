...
import { runProcessDefinition, extractTaskInfoFromObject,addMoreInfoToTask} from './processAction';
let data = {
    taskInfo: {},
    originData: {}
};
...
let variableProcess = [];
export const getFirstNodeData = async function (_paramId){
        ...
        if(documentToStart && documentToStart != 'null' ){
            ...
            try {
                ...
                let newProcessInstance = await runProcessDefinition(null, processDef, [], instanceName);
                await checkAndGotoMyTask(newProcessInstance);
            } catch (error) {
                showNotify(error)
            }
        }
    ...
    async function checkAndGotoMyTask(newProcessInstance){
        let filter={};
        let arrTask = [];
    ...
    async function getVariablesProcess(processInstanceId){
        ...
        SYMPER_APP.$snotifySuccess("Khởi tạo quy trình thành công!");
    }
