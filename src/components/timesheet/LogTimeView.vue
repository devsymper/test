...
<v-card>
    <v-card-title class=" pt-1 pb-2 headline lighten-2" primary-title>
        <div class="w-100 pb-1" style="border-bottom: 1px solid lightgrey">
            ...
            <span class="fs-16 fw-430 ">
                <span v-if="event.type==1" >{{$t('timesheet.view_log_time')}}</span>
                <span v-else >{{$t('timesheet.view_plan_time')}} </span>
                 {{event.name}}
            </span>
     ...
     <v-card-text style="height: 55px!important ">
       ...
       <br/> {{getCategoryName(event.category)}}
    </v-card-text>
    <v-card-text style="height: 55px!important ">
