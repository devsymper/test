...
export default {
				...
				for(let i = 0; i < data.length - 1 ; i++){
					if(data[i].delegationState == 'PENDING'){
						if(data[i+1].delegationState == "RESOLVED"){
							let obj = self.createHistoryTask(data[i],data[i+1],"delegate","assign")
						...
						if(data[i+1].delegationState == "PENDING"){
							let obj = self.createHistoryTask(data[i],data[i+1],"assign","delegate")
							...
							let obj = self.createHistoryTask(data[i],data[i+1],"assign","complete")
							let arr = data[i].assignee.split(":")
							obj.userCreateAction  = mapIdToUser[arr[0]].displayName
							newData.push(obj)
						}else if(data[i].assignee != data[i+1].assignne){
							let obj = self.createHistoryTask(data[i],data[i+1],"assign","assign")
							let arr = data[i].assignee.split(":")
							obj.userCreateAction  = mapIdToUser[arr[0]].displayName
							obj.userReceiveAction  = mapIdToUser[data[i+1].assignee].displayName
						}else if(!data[i+1].assignee && !data[i+1].owner){
							let obj = self.createHistoryTask(data[i],data[i+1],"assign","unAssign")
							let arr = data[i].assignee.split(":")
							obj.userCreateAction  = mapIdToUser[arr[0]].displayName
						}
					}
						...
						let obj = self.createHistoryTask(data[i],data[i+1],"unAssign","assign")
						let arr = data[i].assignee.split(":")
						obj.userCreateAction  = mapIdToUser[arr[0]].displayName
						obj.userReceiveAction  = mapIdToUser[data[i+1].assignee].displayName
					}
	...
	methods:{
		...
		createHistoryTask(x,y,startStatus,endStatus){
			let self = this
			let obj = {}
