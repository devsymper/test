	...
	<div class="d-flex flex-column mt-2 ml-2" >
		<div class="d-flex flex-column mb-2 pt-1 mr-1 ml-1 " v-for="(item, i) in allHistory" :key="i">
			...
			<div class="d-flex mt-1 ml-7">
				...
				<span v-else>Đã bắt đầu quy trình </span>
			</div>
			<div class="d-flex align-center ml-7" v-if="item.startStatus">
				Status
				<v-chip x-small label :color="taskStatus[item.startStatus].color" class="ma-2" text-color="white">
					<span class="fs-11">
				...
				<v-chip x-small label class="ma-2" :color="taskStatus[item.endStatus].color" text-color="white">
					<span class="fs-11">
		...
		allHistory() {
			...
			if (data.length > 1) {
				for (let i = 0; i < data.length - 1; i++) {
					...
					} else if (data[i].delegationState == 'RESOLVED' || !data[i].delegationState) {
	...
	methods: {
		...
		checkUserStartWorkflow(data, mapIdToUser) {
			...
			let obj = {
				...
				userCreateActionType: arr[0] == data[1].assignee ? 'owner' : 'notOwner',
