        ...
        <v-card class="card-task">
            <div class="ml-1">
                <div class="mt-1" style="white-space: nowrap;
                    ...
                    <v-icon style="font-size:16px">{{displayIcon(taskSelected.description)}}</v-icon>
            ...
            <div class="ml-1">
                <div style="white-space: nowrap;
                    ...
                    <span class="pl-1" style=" font-size:13px">{{taskSelected.processDefinitionName}}</span>
                </div>
                <div class="pa-0 grey--text mt-1 lighten-2 d-flex justify-space-between">
                        <div
                            class="fs-11 ml-5 text-ellipsis"
                        >{{showAppName()}}</div>

                        <div class="fs-11 py-0 pr-2 text-ellipsis">
                            {{taskSelected.createTime ? $moment(taskSelected.createTime).format('DD/MM/YY HH:mm'):$moment(taskSelected.endTime).format('DD/MM/YY HH:mm')}}
            ...
            <div class="ml-1">
                <div class="pa-0 mt-1 lighten-2 d-flex justify-space-between">
                    <div
                        class="fs-13 userRelated"
                    >
                        <v-icon class="pt-2" style="font-size:14px">mdi-account-check-outline</v-icon>
            ...
            <v-card-actions>
                <span>{{commentCountPerTask['task:' + taskSelected.id]}}</span>
                <span class="pl-1">
                    <v-icon class="fs-14" >mdi-comment-processing-outline</v-icon>
                </span>
                <span class="ml-3">{{fileCountPerTask['task:' + taskSelected.id]}}</span>
                <span class="pl-1">
