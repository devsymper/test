var dashboardLayoutManager = {
    ...
    fillUnaddCells(map, layout, cell) {
        ...
        for (let item of layout) {
            ...
            for (let y = item.y - cell.h + 1; y < item.y + item.h; y++) {
                map = this.fillRowWithNumber(item.x - cell.w + 1, y, cell.w - 1, map);
            }
        }
        return map;
    },
    ...
    getNewCoord(currentLayout, cellSize) {
        ...
        let workspaceMap = this.generateWorkspaceMap(workspaceSize.w, workspaceSize.h);
        workspaceMap = this.fillUnaddCells(workspaceMap, currentLayout, cellSize);
 ...
 * @param {Array} currentLayout Layout hiện tại của dashboard
 */
export const getNewCellConfigLayout = function(cellType, currentLayout, cellId = false, cellSize = {}, active = false) {
    let time = Date.now();
