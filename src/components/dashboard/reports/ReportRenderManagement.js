...
export default class ReportRenderManagement {
     ...
     */
    renderCellsInViewport(scrollY, layout, tabKey, viewportHeight){
        this.initParams(scrollY, layout, tabKey, viewportHeight);
        let categorizedCells = this.categorizeCells();
        this.activeCells(categorizedCells.visual);
    }
    ...
    categorizeCells(){
        ...
        let bottomBorderY = this.scrollY + Number(this.viewportHeight.replace('px',''));
        for(let cellId in cellsSize){
            ...
            if(     
                ...
                ((item.y + item.h) >= topBorderY) && ((item.y + item.h) <= bottomBorderY) || // bottom left của cell nằm trong khung
    ...
    getCellsInfoInActiveTab(){
        ...
        for(let cell of currentLayout){
            ...
            let container = $(`.symper-dashboard-cell-wrapper[symper-cell-id=${cellId}]`)[0];
