...
export default class CrossFilterManagement {
    ...
    makeConditionForAllDataset(initDatasetId){
        let mapDatasetCondition = {};
        let realation = Object.values(this.maps)[0];
        if(!realation){
            return mapDatasetCondition;
        }
        let mapLinks = realation.mapLinksFromDatasets;
        ...
        for(let relateDatasetId in mapLinks[initDatasetId]){
            for(let link of mapLinks[initDatasetId][relateDatasetId]){
                initInfo.colConds[link.fromCol] = `SELECT DISTINCT ${link.fromCol} FROM {${tbNamePlaceHolder}} WHERE {SYMPER_INIT_CONDITION}`;
            }
        }
    ...
    makeConditionForCols(link, mapDatasetCondition, setCols){
        ...
        if(!setCols[nextCol] && !(dtsInfo.dtsConds[0] + '') != '{SYMPER_INIT_CONDITION}'){
            let tbNamePlaceHolder = this.getTableNamePlaceHolder(link.toDataset);
            let colCond = mapDatasetCondition[link.fromDataset].colConds[link.fromCol];
            colCond = `{${nextCol}} IN (${colCond})`;
            dtsInfo.dtsConds.push(colCond);
            dtsInfo.colConds[link.toCol] = ` SELECT {${nextCol}} FROM {${tbNamePlaceHolder}} WHERE ${colCond}`;
    ...
    checkSetLink(setLinks, link){
        let s1 = link.fromDataset + '_' + link.toCol;
