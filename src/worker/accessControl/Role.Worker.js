...
self.onmessage = async function (event) {
	...
	switch (action) {
        case 'getDetailSystemRole':
            ...
            postMessage({action:'getDetailSystemRole', dataAfter : getDetailSystemRoleRes})
            break;
        case 'deleteRole':
...
export const getDetailSystemRole = async function(data) {
	...
	let resPermission = await permissionApi.getPermissionOfRole(data.getPermissionOfRole)
	return {
		dataSystemRole: res,
...
export const deleteRole = async function(ids) {
	let res = await systemRoleApi.deleteRole(ids[0]);
	if(res.status == 200){
		return 'success'
	}else{
		return 'error'
