...
self.onmessage = async function (event) {
	...
	switch (action) {
        case 'deletePermission':
            ...
            postMessage({action:'deletePermission', dataAfter : deletePermissionRes})
            break;
        case 'getActionPackOfPermission':
			let getActionPackOfPermissionRes = await getActionPackOfPermission(data.packId);
            postMessage({action:'getActionPackOfPermission', dataAfter : getActionPackOfPermissionRes})
            break;
        case 'updatePermission':
			let updatePermissionRes = await updatePermission(data);
            postMessage({action:'updatePermission', dataAfter : updatePermissionRes})
            break;
        case 'createPermission':
			let createPermissionRes = await createPermission(data.dataToSave);
            postMessage({action:'createPermission', dataAfter : createPermissionRes})
            break;
        default:
            break;
    }
...
export const deletePermission = async function(ids) {
	let res = await permissionApi.deletePermissionPackBeta(ids[0]);
	if(res.status == 200){
		return 'success'
	}else{
		return 'error'
