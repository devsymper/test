...
self.onmessage = async function (event) {
	...
	switch (action) {
        case 'getMappingTable':
			...
			await postMessage({action:'getMappingTable', dataAfter : mapping})
            break;
        case 'getLastData':
...
export const getMappingTable = async function (data){
    ...
    if(column.length>2){
        for(let i = 0; i<data.tables.length;i++){
            for(let j=0; j<data.lastTable.length;j++){
                if(data.lastTable[j].nameTable==data.tables[i].name&&checkNameSheetExist(data.lastTable[j].nameSheet, data.nameSheets)){
                    for(let k = 0; k<data.tables[i].controls.length;k++){
                       if(data.tables[i].controls[k].name==data.lastTable[j].controlName){
                            nameSheets = hideNameSheet(data.lastTable[j].nameSheet);
                            data.tables[i].sheetMap = {name:'',enable:false};
                            data.tables[i].controls[k].dataColumn = {name: '',index: 0, enable:false};
    ...
    setLastKeyGeneral(data.tables,data.lastTable,data.lastKey, data.nameColumnDetail);
    data.tables = setLastKeyTables().tables;
     newLastKeyTables = setLastKeyTables().newLastKeyTables
...
export const getIndexColumnMapping = function(value, nameSheet,nameColumnDetail){
...
export const setLastKeyGeneral = function(tables, lastTable, lastKey,nameColumnDetail){
...
export const setLastKeyTables = function(lastKey,nameSheets,nameColumnDetail ){
    // kiểm tra để lấy key
    let newLastKeyTables = [];
    ...
    for(let i = 1; i<tables.length; i++){
           for(let j = 0; j<newLastKeyTables.length; j++){
                if(tables[i].sheetMap&&tables[i].sheetMap.name==newLastKeyTables[j].sheet){
