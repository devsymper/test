...
var handler = {
	...
	async getRelationConfigs(data){
		...
		obj.items = handler.getRappidItems(configData.columns, configData.subDatasets);
		...
		obj.links = configData.links.reduce((l,ele)=>{
			l.push({
				...
				uid: ele.uid,
				source: { id: subDtsToDtsId[ele.id_dataset1], port: ele.id_dataset1+'_'+ele.col1_name },
				target: { id: subDtsToDtsId[ele.id_dataset2], port: ele.id_dataset2+'_'+ele.col2_name }
			});
			return l;
		},[]);
