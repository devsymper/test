...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      permissionSelected: ['Config','hoang'],
      groups : [],
      itemsGroup :[
        'Thành viên',
        'Quản lí',
    ...
    methods: {
      ...
      getDetailUser(id){
        ...
        userApi
          ...
          .then(res => {
              if (res.status == 200) {
                  thisCpn.setDetailUser(res.data.user);
              }
          })
          .catch(err => {
              console.log("error from list group user api!!!", res);
      ...
      setDetailUser(user){
        ...
        this.note = user['note'];
        this.permissionSelected = user['permission'];
        this.active = (user['status'] == "1" || user['status'] == 1) ? true : false;
