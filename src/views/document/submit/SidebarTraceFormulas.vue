		...
		<VuePerfectScrollbar style="calc(100% - 62px);">
             <v-stepper alt-labels>
                <v-stepper-header>
                    ...
                    <v-stepper-step step="1" :color="'#0760d980'">{{$t('document.submit.panel.traceCurrent')}}</v-stepper-step>

                    <v-divider></v-divider>

                    <v-stepper-step step="1" :color="'#40813780'">{{$t('document.submit.panel.traceOutput')}}</v-stepper-step>
                </v-stepper-header>
                </v-stepper>
			<v-expansion-panels
			multiple
...
import {getListInputInDocument,mapTypeToEffectedControl} from './../common/common'
...
<style scoped>
	...
	.s-trace-control-panel ::v-deep .v-expansion-panel--active + .v-expansion-panel{
    ...
    .sidebar-trace-control ::v-deep .v-stepper--alt-labels .v-stepper__header .v-divider{
