    ...
    margin-bottom: -3px;" :src="dataControl.icon" alt="">
                ...
                >
                    <template v-slot:selection="{ item }">
                        <img style="height:14px;margin-right: 8px;" :src="item.prop.icon" alt="">
                    <span style="    font-weight: 500;">{{item.prop.title}}</span>
                    </template>
                    <template v-slot:item="{ item,index }">
                        <div :key="index">
                            <img style="height:14px;width:14px;margin-right: 8px;margin-bottom: -2px;" :src="item.prop.icon" alt="">
                    <span style="font-size:13px">{{item.prop.title}}</span>
                        </div>
...
export default {
    ...
    computed:{
        allControl(){
            let listControl = getAllControlForTableSetting(['label','image','qrCode','textInput','number','date','dateTime','time','month','phone','select','documentSelect','color','hidden','email','user','currency','percent']);
            return listControl;
        },
        editorStore(){  
