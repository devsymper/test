	...
	<div :style="{ position: isExpand ? 'relative' : 'absolute' }" class="report-sidebar" @mouseenter="drawer = true" @mouseleave="drawer = false">
		...
		<transition name="slide">
			<div v-show="checkShowSideBar()" :class="{ 'wrapper-sidebar': true, 'wrapper-sidebar-mini': !isExpand, 'wrapper-sidebar-full': isExpand }">
				<div class="font-weight-bold  ml-2" style="font-size: 18px !important; margin-top: 8px">
					Báo cáo
				</div>
				<VuePerfectScrollbar class="sidebar-content">
					<v-list :expand="true" dense>
						...
						>
							...
							<v-icon slot="appendIcon" small :symper-action="$bindAction(item.action)"> </v-icon>
							<template v-slot:activator v-if="item.title">
								<v-list-item-title style="margin-left:-25px;" :symper-action="$bindAction(item.action)">
									<v-list-item-title class=" ml-3" style="color:rgb(0,0,0,0.8)" :symper-action="$bindAction(item.action)">
...
export default {
	created() {
		this.$evtBus.$on('symper-app-wrapper-clicked', (evt) => {
			if (this.$refs.projectPopupView && $(evt.target).closest('.project-info').length == 0 && $(evt.target).closest('.popup-select-project').length == 0) {
				this.$refs.projectPopupView.hide();
			}
			if (this.$refs.SelectBoard && $(evt.target).closest('.v-list-item').length == 0 && $(evt.target).closest('.card-add-board').length == 0) {
