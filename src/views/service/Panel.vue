<template>
    <div class="h-100">
        <h3 class="header-title" v-if="actionType == 'add'">{{ $t('service.panel.add.title')}}</h3>
        <h3 class="header-title" v-if="actionType == 'edit'">{{ $t('service.panel.edit.title')}}</h3>
        <v-divider></v-divider>
        <div class="form-add-service-content">
            <form-tpl :singleLine="true" :labelWidth="`160px`"  :allInputs="allInputs"/>
...
export default {
    ...
    methods:{
        ...
        addService(dataPost){
            ...
            serviceApi.add(dataPost).then(res => {
				if (res.status == 200) {
        ...
        editService(dataPost){
            ...
            serviceApi.edit(dataPost).then(res => {
				if (res.status == 200) {
                    thisCpn.$emit('refresh-list');
				}
			})
        ...
        setData(service){
            ...
            this.$refs.repeatTime.setData(service.loop_config,service.is_repeat);
        },
        openPanel(){
            this.actionType = 'add';
            this.resetData();
